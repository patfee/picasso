...
def create_app():
    ...
    @app.callback(Output("main-range", "children"),
                  Output("fold-range", "children"),
                  Input("main-angles", "data"), Input("fold-angles", "data"))
    ...
    # ⬇️ Envelope type explanation callback
    ...
    def explain_envelope(kind):
        if kind == "concave":
            return html.Div([
                ...
                html.Span(
                    "Tightly wraps the points and follows inward bends (concavities). "
                    "Captures a closer 'true' working envelope but can be sensitive to gaps or noise. "
                    "Uses a fast approximation with subsampling."
                )
            ])
        return html.Div([
            html.B("Convex hull: "),
            html.Span(
                "Smallest convex polygon enclosing all points. Ignores concavities, "
                "so it usually overestimates area (conservative) but is very fast and stable."
            )
        ])
    ...
    def update_figure(main_step, fold_step, pedestal_value, envelope_value,
        ...
        _, _, _, _, pts = resample_grid(np.array(fold_angles_store),
                                        np.array(main_angles_store),
        ...
        dense_xy = dense_xy[~np.isnan(dense_xy).any(axis
